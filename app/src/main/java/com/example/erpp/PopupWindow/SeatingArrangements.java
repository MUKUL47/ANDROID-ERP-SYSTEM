...
public class SeatingArrangements extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        arrangement = findViewById(R.id.noticeF);
    ...
    private void updateSeatingArrangements() {
        FirebaseDatabase.getInstance().getReference().child("StudentLogs/"+ID).child("seatingArrangement").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if( arrangements.size() == 0 ){
                    Toast.makeText(SeatingArrangements.this,"No seating arrangement[s] found",Toast.LENGTH_LONG).show();
                }
    ...
    public void exitSeatinArrangement(View view) {
        onBackPressed();
    }
}
